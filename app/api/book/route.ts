...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!belongsToWindow) return Response.json({ error: "Horário fora da disponibilidade." }, { status: 400 });
    ...
    if (conflict) return Response.json({ error: "Horário já reservado." }, { status: 409 });
